...
Gtk::Window* qDialog = nullptr;

static void on_button_clicked()
{
    if (qDialog) {
        qDialog->hide(); // will cause main::run() to end
    }
}
...
int main(const int argc, const char* const argv[])
{
    auto gtkapp = Gtk::Application::create(
        const_cast<int&>(argc), const_cast<char**&>(argv),
        "08.gtk.com"
    );
    ...
    try 
    {
        refBuilder->add_from_file(
            "/home/sam/workspace/cpp/ogl_playground/08/assets/window.glade"
        );
    }
    ...
    t = std::thread(app.threaded_run, &app);*/

    refBuilder->get_widget("window", qDialog);
    if (qDialog) {
        Gtk::Button* qButton{nullptr};
        refBuilder->get_widget("quit_button", qButton);
        if (qButton) {
            qButton->signal_clicked().connect(sigc::ptr_fun(on_button_clicked));
        }
